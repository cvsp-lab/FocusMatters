    ...
    def evolve_vcd_sampling(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if use_ours_cd:
                original_set = []
                if 'llava' in LVLM_backbone.model_name:
                    for i in range(22):
                        original_set.append(LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers[i].self_attn.forward)
                    for idx, layer in enumerate(LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers):
                        layer.self_attn.masking = None
                    for ii in range(22):
                        LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers[ii].self_attn.forward = types.MethodType(forward_masking, LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers[ii].self_attn)
                    for kk in range(12, 17):
                        LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers[kk].self_attn.masking = ours_mask_cd
                elif 'shikra' in LVLM_backbone.model_name:
                    for i in range(22):
                        original_set.append(LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers[i].self_attn.forward)
                    for idx, layer in enumerate(LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers):
                        layer.self_attn.masking = None
                    for ii in range(22):
                        LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers[ii].self_attn.forward = types.MethodType(forward_masking, LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers[ii].self_attn)
                    for kk in range(12, 17):
                        LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers[kk].self_attn.masking = ours_mask_cd
                elif 'minigpt4' in LVLM_backbone.model_name:
                    for layer in LVLM_backbone.visual_encoder.blocks:
                        original_set.append(layer.attn.forward)
                        layer.attn.masking = None
                        layer.attn.forward = types.MethodType(forward_masking, layer.attn)
                    for kk in range(8, 16):
                        LVLM_backbone.visual_encoder.blocks[kk].attn.masking = ours_mask_cd
            ...
            if use_ours_cd:
                # print(LVLM_backbone.model_name)
                if 'llava' in LVLM_backbone.model_name:
                    for ii in range(22):
                        LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers[ii].self_attn.masking = None
                        LVLM_backbone.llama_model.model.vision_tower.vision_tower.vision_model.encoder.layers[ii].self_attn.forward = original_set[ii]
                elif 'shikra' in LVLM_backbone.model_name:
                    for ii in range(22):
                        LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers[ii].self_attn.masking = None
                        LVLM_backbone.llama_model.model.vision_tower[0].vision_model.encoder.layers[ii].self_attn.forward = original_set[ii]
                elif 'minigpt4' in LVLM_backbone.model_name:
                    for ii, layer in enumerate(LVLM_backbone.visual_encoder.blocks):
                        layer.attn.masking = None
                        layer.attn.forward = original_set[ii]
