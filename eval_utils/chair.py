...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
            ...
            num_gt_objects = 0.
            # Add variables for precision calculation
            num_correct_mentioned_objects = 0.
            num_mentioned_objects = 0.
            output = {'sentences': []}
            for i in tqdm.trange(len(caps)):
                ...
                for word, node_word, idx in zip(words, node_words, idxs):
                    if node_word not in gt_objects:
                        ...
                        correct_mentioned.add(node_word)
                ...
                mentioned_objects = set(node_words)
                num_mentioned_objects += len(mentioned_objects)
                num_correct_mentioned_objects += len(correct_mentioned)
                ...
                if len(mentioned_objects) > 0:
                    cap_dict['metrics']['Precision'] = len(correct_mentioned) / len(mentioned_objects)
                # Calculate F1 for this caption
                precision = cap_dict['metrics']['Precision']
                recall = cap_dict['metrics']['Recall']
                if precision + recall > 0:
                    cap_dict['metrics']['F1'] = 2 * (precision * recall) / (precision + recall)
                output['sentences'].append(cap_dict)
            chair_s = (num_hallucinated_caps/num_caps)
            chair_i = (hallucinated_word_count/coco_word_count)
            recall = num_recall_gt_objects / num_gt_objects
            # Calculate overall precision
            precision = num_correct_mentioned_objects / num_mentioned_objects if num_mentioned_objects > 0 else 0
...
def print_metrics(hallucination_cap_dict, save_result_file, quiet=False):
    ...
    with open(save_result_file, 'w') as f:
        json.dump(sentence_metrics, f, indent=2, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--chair_result_file", type=str, default='',
                        help="path towards json or jsonl saving CHAIR results.")                    
    ...
    # args.cap_file = os.path.join('chair_results', args.cap_file)
    # args.save_path = os.path.join('chair_results', args.save_path)
    ...
    print_metrics(cap_dict, args.chair_result_file)
